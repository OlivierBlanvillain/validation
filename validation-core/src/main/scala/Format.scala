...
object Format {
  ...
  implicit def functionalInvariantSyntaxFormat[IR, IW : Monoid](implicit rcb: SyntaxCombine[Rule[IR, ?]], wcb: SyntaxCombine[Write[?, IW]]): SyntaxCombine[Format[IR, IW, ?]] =
  ...
  implicit def fboFormat[IR, IW : Monoid, O](f: Format[IR, IW, O])(implicit fcb: SyntaxCombine[Format[IR, IW, ?]]): InvariantSyntaxObs[Format[IR, IW, ?], O] =
