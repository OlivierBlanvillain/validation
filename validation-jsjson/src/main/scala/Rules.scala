...
object Rules extends DefaultRules[js.Dynamic] {
  ...
  implicit def bigDecimal = jsonAs[BigDecimal] {
    case v if Try(BigDecimal(v.toString)).isSuccess => Valid(BigDecimal(v.toString))
  }("error.number", "BigDecimal")

  import java.{math => jm}
  implicit def javaBigDecimal = jsonAs[jm.BigDecimal] {
    case v if Try(new jm.BigDecimal(v.toString)).isSuccess => Valid(new jm.BigDecimal(v.toString))
  }("error.number", "BigDecimal")
