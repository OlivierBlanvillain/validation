...
object Writes extends DefaultWrites[JsValue] with GenericWrites[JsValue] {
  ...
  implicit val validationErrorW: Write[ValidationError, JsValue] = Write[ValidationError, JsValue] { err =>
    Json.obj(
      "msg" -> JsString(err.message),
      "args" -> err.args.foldLeft(Json.arr()) { (arr, arg) => arr :+
        (arg match {
  ...
  implicit def failureW(implicit w: Write[(Path, Seq[ValidationError]), JsValue])
  ...
  private def tToJs[T]: Write[T, JsValue] = Write[T, JsValue]((i: T) => JsNumber(BigDecimal(i.toString)))
  ...
  implicit val bigDecimalW: Write[BigDecimal, JsValue] = Write[BigDecimal, JsValue](JsNumber.apply)
  implicit def booleanW: Write[Boolean, JsValue] = Write[Boolean, JsValue](JsBoolean.apply)
  ...
  implicit def optionW[I](implicit w: Write[I, JsValue]): Write[Option[I], JsValue] =
    Write {
      case None    => JsNull
