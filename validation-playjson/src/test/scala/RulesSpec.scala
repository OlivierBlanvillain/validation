...
class RulesSpec extends WordSpec with Matchers {
  ...
  "Json Rules" should {
    ...
    "extract data" in {
      (Path \ "firstname").read[JsValue, String].validate(valid) shouldBe(Valid("Julien"))
    ...
    "support checked" in {
      ...
      p.from[JsValue](checked).validate(Json.obj()) shouldBe(Invalid(Seq(Path \ "issmth" -> Seq(ValidationError("error.required")))))
    ...
    "support all types of Json values" when {
      ...
      "Int" in {
        ...
        (Path \ "n" \ "o" \ "p" ).read[JsValue, Int].validate(Json.obj("n" -> Json.obj("o" -> Json.obj("p" -> 4)))) shouldBe(Valid(4))
        (Path \ "n" \ "o" \ "p").read[JsValue, Int].validate(Json.obj("n" -> Json.obj("o" -> Json.obj("p" -> "foo")))) shouldBe(Invalid(Seq(Path \ "n" \ "o" \ "p" -> Seq(ValidationError("error.number", "Int")))))

        val errPath = Path \ "foo"
        val error = Invalid(Seq(errPath -> Seq(ValidationError("error.required"))))
        errPath.read[JsValue, Int].validate(Json.obj("n" -> 4)) shouldBe(error)
      }
      ...
      "String" in {
        (Path \ "n").read[JsValue, String].validate(Json.obj("n" -> "foo")) shouldBe(Valid("foo"))
        (Path \ "n").read[JsValue, String].validate(Json.obj("n" -> 42)) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "String")))))
        (Path \ "n").read[JsValue, String].validate(Json.obj("n" -> Seq("foo"))) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "String")))))
        (Path \ "o").read[JsValue, String].validate(Json.obj("o" -> Json.obj("n" -> "foo"))) shouldBe(Invalid(Seq(Path \ "o" -> Seq(ValidationError("error.invalid", "String")))))
      }

      "JsObject" in {
        (Path \ "o").read[JsValue, JsObject].validate(Json.obj("o" -> Json.obj("n" -> "foo"))) shouldBe(Valid(JsObject(Seq("n" -> JsString("foo")))))
        (Path \ "n").read[JsValue, JsObject].validate(Json.obj("n" -> 42)) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Object")))))
        (Path \ "n").read[JsValue, JsObject].validate(Json.obj("n" -> "foo")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Object")))))
        (Path \ "n").read[JsValue, JsObject].validate(Json.obj("n" -> Seq("foo"))) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Object")))))
      }

      "JsString" in {
        (Path \ "n").read[JsValue, JsString].validate(Json.obj("n" -> "foo")) shouldBe(Valid(JsString("foo")))
        (Path \ "n").read[JsValue, JsString].validate(Json.obj("n" -> 42)) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "String")))))
      }
      ...
      "JsBoolean" in {
        (Path \ "n").read[JsValue, JsBoolean].validate(Json.obj("n" -> true)) shouldBe(Valid(JsBoolean(true)))
        (Path \ "n").read[JsValue, JsBoolean].validate(Json.obj("n" -> "foo")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Boolean")))))
      }

      "Option" in {
        (Path \ "n").read[JsValue, Option[Boolean]].validate(Json.obj("n" -> true)) shouldBe(Valid(Some(true)))
        (Path \ "n").read[JsValue, Option[Boolean]].validate(Json.obj("n" -> JsNull)) shouldBe(Valid(None))
        (Path \ "n").read[JsValue, Option[Boolean]].validate(Json.obj("foo" -> "bar")) shouldBe(Valid(None))
        (Path \ "n").read[JsValue, Option[Boolean]].validate(Json.obj("n" -> "bar")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Boolean")))))
      }

      "Map[String, V]" in {
        (Path \ "n").read[JsValue, Map[String, String]].validate(Json.obj("n" -> Json.obj("foo" -> "bar"))) shouldBe(Valid(Map("foo" -> "bar")))
        (Path \ "n").read[JsValue, Map[String, Int]].validate(Json.obj("n" -> Json.obj("foo" -> 4, "bar" -> 5))) shouldBe(Valid(Map("foo" -> 4, "bar" -> 5)))
        (Path \ "x").read[JsValue, Map[String, Int]].validate(Json.obj("n" -> Json.obj("foo" -> 4, "bar" -> "frack"))) shouldBe(Invalid(Seq(Path \ "x" -> Seq(ValidationError("error.required")))))
        (Path \ "n").read[JsValue, Map[String, Int]].validate(Json.obj("n" -> Json.obj("foo" -> 4, "bar" -> "frack"))) shouldBe(Invalid(Seq(Path \ "n" \ "bar" -> Seq(ValidationError("error.number", "Int")))))
      }

      "Traversable" in {
        (Path \ "n").read[JsValue, Traversable[String]].validate(Json.obj("n" -> Seq("foo"))).toOption.get.toSeq shouldBe(Seq("foo"))
        (Path \ "n").read[JsValue, Traversable[Int]].validate(Json.obj("n" -> Seq(1, 2, 3))).toOption.get.toSeq shouldBe(Seq(1, 2, 3))
        (Path \ "n").read[JsValue, Traversable[String]].validate(Json.obj("n" -> "paf")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Array")))))
      }

      "Array" in {
        (Path \ "n").read[JsValue, Array[String]].validate(Json.obj("n" -> Seq("foo"))).toOption.get.toSeq shouldBe(Seq("foo"))
        (Path \ "n").read[JsValue, Array[Int]].validate(Json.obj("n" -> Seq(1, 2, 3))).toOption.get.toSeq shouldBe(Seq(1, 2, 3))
        (Path \ "n").read[JsValue, Array[String]].validate(Json.obj("n" -> "paf")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Array")))))
      }

      "Seq" in {
        (Path \ "n").read[JsValue, Seq[String]].validate(Json.obj("n" -> Seq("foo"))).toOption.get shouldBe(Seq("foo"))
        (Path \ "n").read[JsValue, Seq[Int]].validate(Json.obj("n" -> Seq(1, 2, 3))).toOption.get shouldBe(Seq(1, 2, 3))
        (Path \ "n").read[JsValue, Seq[String]].validate(Json.obj("n" -> "paf")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Array")))))
        (Path \ "n").read[JsValue, Seq[String]].validate(JsObject(Seq("n" -> JsArray(Seq(JsString("foo"), JsNumber(2)))))) shouldBe(Invalid(Seq(Path \ "n" \ 1 -> Seq(ValidationError("error.invalid", "String")))))
      }

    }

    "validate data" in {
      (Path \ "firstname").from[JsValue](notEmpty).validate(valid) shouldBe(Valid("Julien"))

      val p = (Path \ "informations" \ "label")
      p.from[JsValue](notEmpty).validate(valid) shouldBe(Valid("Personal"))
      p.from[JsValue](notEmpty).validate(invalid) shouldBe(Invalid(Seq(p -> Seq(ValidationError("error.required")))))
    }

    "validate optional" in {
      (Path \ "firstname").read[JsValue, Option[String]].validate(valid) shouldBe(Valid(Some("Julien")))
      (Path \ "foobar").read[JsValue, Option[String]].validate(valid) shouldBe(Valid(None))
    }
    ...
    "coerce type" in {
      ...
      (Path \ "age").from[JsValue](min(20)).validate(valid) shouldBe(Valid(27))
      (Path \ "age").from[JsValue](max(50)).validate(valid) shouldBe(Valid(27))
      (Path \ "age").from[JsValue](min(50)).validate(valid) shouldBe(Invalid(Seq((Path \ "age") -> Seq(ValidationError("error.min", 50)))))
      (Path \ "age").from[JsValue](max(0)).validate(valid) shouldBe(Invalid(Seq((Path \ "age") -> Seq(ValidationError("error.max", 0)))))
      (Path \ "firstname").read[JsValue, Int].validate(valid) shouldBe(Invalid(Seq((Path \ "firstname") -> Seq(ValidationError("error.number", "Int")))))
    }

    "compose constraints" in {
      val composed = notEmpty |+| minLength(3)
      (Path \ "firstname").from[JsValue](composed).validate(valid) shouldBe(Valid("Julien"))
    ...
    "lift validations to seq validations" in {
      ...
      (Path \ "n").from[JsValue](seqR(notEmpty))
        .validate(Json.obj("n" -> Seq("foo", ""))) shouldBe(Invalid(Seq(Path \ "n" \ 1 -> Seq(ValidationError("error.required")))))
    }
