...
class RulesSpec extends WordSpec with Matchers {
  ...
  "Rules" should {
    ...
    "support checked" in {
      ...
      p.from[UrlFormEncoded](checked).validate(Map.empty) shouldBe(Invalid(Seq(Path \ "issmth" -> Seq(ValidationError("error.required")))))
      p.from[UrlFormEncoded](checked).validate(Map("issmth" -> Seq("false"))) shouldBe(Invalid(Seq(Path \ "issmth" -> Seq(ValidationError("error.equals", true)))))
    }
    ...
    "support primitives types" when {
      ...
      "Int" in {
        ...
        From[UrlFormEncoded] { __ => (__ \ "n" \ "o" \ "p").read[Int] }.validate(Map("n.o.p" -> Seq("4"))) shouldBe(Valid(4))
        From[UrlFormEncoded] { __ => (__ \ "n" \ "o" \ "p").read[Int] }.validate(Map("n.o.p" -> Seq("foo"))) shouldBe(Invalid(Seq(Path \ "n" \ "o" \ "p" -> Seq(ValidationError("error.number", "Int")))))

        val errPath = Path \ "foo"
        val error = Invalid(Seq(errPath -> Seq(ValidationError("error.required"))))
        From[UrlFormEncoded] { __ => (__ \ "foo").read[Int] }.validate(Map("n" -> Seq("4"))) shouldBe(error)
      }
      ...
      "Boolean" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[Boolean] }.validate(Map("n" -> Seq("true"))) shouldBe(Valid(true))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Boolean] }.validate(Map("n" -> Seq("TRUE"))) shouldBe(Valid(true))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Boolean] }.validate(Map("n" -> Seq("foo"))) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Boolean")))))
      }

      "String" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[String] }.validate(Map("n" -> Seq("foo"))) shouldBe(Valid("foo"))
        From[UrlFormEncoded] { __ => (__ \ "o").read[String] }.validate(Map("o.n" -> Seq("foo"))) shouldBe(Invalid(Seq(Path \ "o" -> Seq(ValidationError("error.required")))))
      }

      "Option" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[Option[Boolean]] }.validate(Map("n" -> Seq("true"))) shouldBe(Valid(Some(true)))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Option[Boolean]] }.validate(Map("n" -> Seq(""))) shouldBe(Valid(None))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Option[Boolean]] }.validate(Map("foo" -> Seq("bar"))) shouldBe(Valid(None))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Option[Boolean]] }.validate(Map("n" -> Seq("bar"))) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Boolean")))))
      }

      "Map[String, Seq[V]]" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[Map[String, Seq[String]]] }.validate(Map("n.foo" -> Seq("bar"))) shouldBe(Valid(Map("foo" -> Seq("bar"))))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Map[String, Seq[Int]]] }.validate(Map("n.foo" -> Seq("4"), "n.bar" -> Seq("5"))) shouldBe(Valid(Map("foo" -> Seq(4), "bar" -> Seq(5))))
        From[UrlFormEncoded] { __ => (__ \ "x").read[Map[String, Int]] }.validate(Map("n.foo" -> Seq("4"), "n.bar" -> Seq("frack"))) shouldBe(Valid(Map.empty))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Map[String, Seq[Int]]] }.validate(Map("n.foo" -> Seq("4"), "n.bar" -> Seq("frack"))) shouldBe(Invalid(Seq(Path \ "n" \ "bar" \ 0 -> Seq(ValidationError("error.number", "Int")))))
      }

      "Traversable" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[Traversable[String]] }.validate(Map("n" -> Seq("foo"))).toOption.get.toSeq shouldBe(Seq("foo"))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Traversable[Int]] }.validate(Map("n[]" -> Seq("1", "2", "3"))).toOption.get.toSeq shouldBe(Seq(1, 2, 3))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Traversable[Int]] }.validate(Map("n[]" -> Seq("1", "paf"))) shouldBe(Invalid(Seq(Path \ "n" \ 1 -> Seq(ValidationError("error.number", "Int")))))
      }

      "Array" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[Array[String]] }.validate(Map("n" -> Seq("foo"))).toOption.get.toSeq shouldBe(Seq("foo"))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Array[Int]] }.validate(Map("n[]" -> Seq("1", "2", "3"))).toOption.get.toSeq shouldBe(Seq(1, 2, 3))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Array[Int]] }.validate(Map("n[]" -> Seq("1", "paf"))) shouldBe(Invalid(Seq(Path \ "n" \ 1 -> Seq(ValidationError("error.number", "Int")))))
      }

      "Seq" in {
        From[UrlFormEncoded] { __ => (__ \ "n").read[Seq[String]] }.validate(Map("n" -> Seq("foo"))).toOption.get shouldBe(Seq("foo"))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Seq[Int]] }.validate(Map("n[]" -> Seq("1", "2", "3"))).toOption.get shouldBe(Seq(1, 2, 3))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Seq[Int]] }.validate(Map(
          "n[0]" -> Seq("1"),
          "n[1]" -> Seq("2"),
          "n[3]" -> Seq("3")
        )).toOption.get shouldBe(Seq(1, 2, 3))
        From[UrlFormEncoded] { __ => (__ \ "n").read[Seq[Int]] }.validate(Map("n[]" -> Seq("1", "paf"))) shouldBe(Invalid(Seq(Path \ "n" \ 1 -> Seq(ValidationError("error.number", "Int")))))
      }
    }

    "validate data" in {
      From[UrlFormEncoded] { __ => (__  \ "firstname").read(notEmpty) }.validate(valid) shouldBe(Valid("Julien"))

      val p = (Path \ "informations" \ "label")
      From[UrlFormEncoded] { __ => (__  \ "informations" \ "label").read(notEmpty) }.validate(valid) shouldBe(Valid("Personal"))
      From[UrlFormEncoded] { __ => (__  \ "informations" \ "label").read(notEmpty) }.validate(invalid) shouldBe(Invalid(Seq(p -> Seq(ValidationError("error.required")))))
    }

    "validate seq" in {
      def isNotEmpty[T <: Traversable[_]] = validateWith[T]("error.notEmpty"){ !_.isEmpty }

      From[UrlFormEncoded] { __ => (__ \ "firstname").read[Seq[String]] }.validate(valid) shouldBe(Valid(Seq("Julien")))
      From[UrlFormEncoded] { __ => (__ \ "foobar").read[Seq[String]] }.validate(valid) shouldBe(Valid(Seq()))
      From[UrlFormEncoded] { __ => (__ \ "foobar").read(isNotEmpty[Seq[Int]]) }.validate(valid) shouldBe(Invalid(Seq(Path \ "foobar" -> Seq(ValidationError("error.notEmpty")))))
    }

    "validate optional" in {
      From[UrlFormEncoded] { __ => (__ \ "firstname").read[Option[String]] }.validate(valid) shouldBe(Valid(Some("Julien")))
      From[UrlFormEncoded] { __ => (__ \ "firstname").read[Option[Int]] }.validate(valid) shouldBe(Invalid(Seq(Path \ "firstname" -> Seq(ValidationError("error.number", "Int")))))
      From[UrlFormEncoded] { __ => (__ \ "foobar").read[Option[String]] }.validate(valid) shouldBe(Valid(None))
    }
    ...
    "coerce type" in {
      From[UrlFormEncoded] { __ => (__ \ "age").read[Int] }.validate(valid) shouldBe(Valid(27))
      From[UrlFormEncoded] { __ => (__ \ "firstname").read[Int] }.validate(valid) shouldBe(Invalid(Seq((Path \ "firstname") -> Seq(ValidationError("error.number", "Int")))))
    }
    ...
    "compose constraints" in {
      ...
      From[UrlFormEncoded] { __ => (__ \ "informations" \ "label").read(composed) }.validate(invalid) shouldBe(err)
    }
    ...
    "perform complex validation" in {
      ...
      contactValidated.validate(invalidM) shouldBe(Invalid(Seq(
