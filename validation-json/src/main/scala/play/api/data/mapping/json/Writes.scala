...
object Writes extends DefaultWrites with DefaultMonoids with GenericWrites[JsValue] {
  ...
  implicit def errors(implicit wErrs: WriteLike[Seq[ValidatedError], JsValue]) =
    Write[(Path, Seq[ValidatedError]), JsObject] {
      case (p, errs) =>
        Json.obj(p.toString -> wErrs.writes(errs))
    }

  implicit def failure(implicit w: WriteLike[(Path, Seq[ValidatedError]), JsObject]) =
    Write[Invalid[Seq[(Path, Seq[ValidatedError])]], JsObject] {
      case Invalid(errs) =>
