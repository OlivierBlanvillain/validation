...
//           (__ \ "phones").read(seqR(notEmpty))) map (ContactInformation.apply _)
//       }
...
//          (__ \ "informations").read(seqR(infoValidation))) map (Contact.apply _)
//       }
...
//           ((__ \ "name").read[String] |@|
//            (__ \ "friends").read[Seq[RecUser]]) map (RecUser.apply _)
//         }
//         w.validate(m) mustEqual Success(u)

//         implicit lazy val w3: Rule[JsValue, User1] = From[JsValue]{ __ =>
//           ((__ \ "name").read[String] |@|
//            (__ \ "friend").read[Option[User1]]) map (User1.apply _)
//         }
//         w3.validate(m1) mustEqual Success(u1)
//       }
