...
class FormatSpec extends WordSpec with Matchers {
  ...
  "Format" should {
    "serialize and deserialize primitives" in {
      ...
      (Path \ "id").from[JsValue](f).validate(Json.obj()) shouldBe(Invalid(Seq(Path \ "id" -> Seq(ValidationError("error.required")))))
    }
    ...
    "support primitives types" when {
      ...
      "Int" in {
        ...
        Formatting[JsValue, JsObject] { __ => (__ \ "n" \ "o" \ "p").format[Int] }.validate(Json.obj("n" -> Json.obj("o" -> Json.obj("p" -> 4)))) shouldBe(Valid(4))
        Formatting[JsValue, JsObject] { __ => (__ \ "n" \ "o" \ "p").format[Int] }.validate(Json.obj("n" -> Json.obj("o" -> Json.obj("p" -> "foo")))) shouldBe(Invalid(Seq(Path \ "n" \ "o" \ "p" -> Seq(ValidationError("error.number", "Int")))))

        val errPath = Path \ "foo"
        val error = Invalid(Seq(errPath -> Seq(ValidationError("error.required"))))
        Formatting[JsValue, JsObject] { __ => (__ \ "foo").format[Int] }.validate(Json.obj("n" -> 4)) shouldBe(error)
      }
      ...
      "Boolean" in {
        ...
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Boolean] }.validate(Json.obj("n" -> "foo")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Boolean")))))
      }

      "String" in {
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[String] }.validate(Json.obj("n" -> "foo")) shouldBe(Valid("foo"))
        Formatting[JsValue, JsObject] { __ => (__ \ "o").format[String] }.validate(Json.obj("o.n" -> "foo")) shouldBe(Invalid(Seq(Path \ "o" -> Seq(ValidationError("error.required")))))
      }

      "Option" in {
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Option[Boolean]] }.validate(Json.obj("n" -> true)) shouldBe(Valid(Some(true)))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Option[Boolean]] }.validate(Json.obj()) shouldBe(Valid(None))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Option[Boolean]] }.validate(Json.obj("foo" -> "bar")) shouldBe(Valid(None))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Option[Boolean]] }.validate(Json.obj("n" -> "bar")) shouldBe(Invalid(Seq(Path \ "n" -> Seq(ValidationError("error.invalid", "Boolean")))))
      }

      "Map[String, Seq[V]]" in {
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Map[String, Seq[String]]] }.validate(Json.obj("n" -> Json.obj("foo" -> Seq("bar")))) shouldBe(Valid(Map("foo" -> Seq("bar"))))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Map[String, Seq[Int]]] }.validate(Json.obj("n" -> Json.obj("foo" -> Seq(4), "bar" -> Seq(5)))) shouldBe(Valid(Map("foo" -> Seq(4), "bar" -> Seq(5))))
        Formatting[JsValue, JsObject] { __ => (__ \ "x").format[Map[String, Int]] }.validate(Json.obj("n" -> Json.obj("foo" -> 4, "bar" -> "frack"))) shouldBe(Invalid(Seq(Path \ "x" -> Seq(ValidationError("error.required")))))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Map[String, Seq[Int]]] }.validate(Json.obj("n" -> Json.obj("foo" -> Seq(4), "bar" -> Seq("frack")))) shouldBe(Invalid(Seq(Path \ "n" \ "bar" \ 0 -> Seq(ValidationError("error.number", "Int")))))
      }

      "Traversable" in {
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Traversable[String]] }.validate(Json.obj("n" -> Seq("foo"))).toOption.get.toSeq shouldBe(Seq("foo"))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Traversable[Int]] }.validate(Json.obj("n" -> Seq(1, 2, 3))).toOption.get.toSeq shouldBe(Seq(1, 2, 3))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Traversable[Int]] }.validate(Json.obj("n" -> Seq("1", "paf"))) shouldBe(Invalid(Seq(
      ...
      "Array" in {
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Array[String]] }.validate(Json.obj("n" -> Seq("foo"))).toOption.get.toSeq shouldBe(Seq("foo"))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Array[Int]] }.validate(Json.obj("n" -> Seq(1, 2, 3))).toOption.get.toSeq shouldBe(Seq(1, 2, 3))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Array[Int]] }.validate(Json.obj("n" -> Seq("1", "paf"))) shouldBe(Invalid(Seq(
          Path \ "n" \ 0 -> Seq(ValidationError("error.number", "Int")),
          Path \ "n" \ 1 -> Seq(ValidationError("error.number", "Int"))
        )))
      }
      ...
      "Seq" in {
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Seq[String]] }.validate(Json.obj("n" -> Seq("foo"))).toOption.get shouldBe(Seq("foo"))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Seq[Int]] }.validate(Json.obj("n" -> Seq(1, 2, 3))).toOption.get shouldBe(Seq(1, 2, 3))
        Formatting[JsValue, JsObject] { __ => (__ \ "n").format[Seq[Int]] }.validate(Json.obj("n" -> Seq("1", "paf"))) shouldBe(Invalid(Seq(
          Path \ "n" \ 0 -> Seq(ValidationError("error.number", "Int")),
          Path \ "n" \ 1 -> Seq(ValidationError("error.number", "Int"))
        )))
      }
    ...
    "format seq" in {
      ...
      Formatting[JsValue, JsObject] { __ => (__ \ "firstname").format[Seq[String]] }.validate(valid) shouldBe(Valid(Seq("Julien")))
      Formatting[JsValue, JsObject] { __ => (__ \ "foobar").format[Seq[String]] }.validate(valid) shouldBe(Valid(Seq()))
    ...
    "be covarianace in the write type" in {
      ...
      f1.validate(MyCat) shouldBe f2.validate(MyCat)
    }
	}
